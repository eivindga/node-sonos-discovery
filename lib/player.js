...
var musicservices = require('./musicservices');
var _discovery;
  ...
  saxParser.on('startNode', function (elem, attr) {
    ...
    if (elem === 'container') {
      ...
      for (var key in attributes) {
        if (mappingAttr[key] === undefined)  continue;
        currentAttr[mappingAttr[key]] = attributes[key] || '';
      }
    }
  });
...
function Player(roomName, descriptorUrl, uuid, discovery) {
  console.log('Initializing player', roomName);
  var _this = this;
  ...
  function updateTransportState(data) {
    ...
    if (data.CurrentTrackMetaData) {
      ...
      // album is sometimes missing, reset it.
      _this.state.currentTrack.album = '';
      _this.state.currentTrack.artist = '';
      _this.state.currentTrack.absoluteAlbumArtURI = '';
...
Player.prototype.soapAction = function (path, action, soap, callback) {
  ...
  var req = http.request({
    ...
    function (res) {
      ...
      if (!callback) return;
      if (res.statusCode !== 200) {
        var msg = 'Expected statusCode 200, but got ' + res.statusCode;
        callback(msg);
        return;
      }
...
Player.prototype.browse = function (objectID, startIndex, requestedCount, callback) {
  ...
  this.soapAction('/MediaServer/ContentDirectory/Control', '"urn:schemas-upnp-org:service:ContentDirectory:1#Browse"', soap, function (error, res) {
...
Player.prototype.getFavorites = function (callback) {
  this.browse('FV:2', null, null, function (error, result) {
    if (error) {
      callback(error);
      return;
    }
...
Player.prototype.getPlaylists = function (callback) {
  this.browse('SQ:', null, null, function (error, result) {
    if (error) {
      callback(error);
      return;
    }
...
Player.prototype.replaceWithFavorite = function (favorite, callback) {
  ...
  _this.getFavorites(function (error, favorites) {
    if (error) {
      _this.log.error('error when fetching favorites');
      return;
    }
...
Player.prototype.addPlaylistToQueue = function (playlistURI, callback) {
  ...
  _this.addURIToQueue(playlistURI, '', function (error) {
    if (error) {
      _this.log.error('problem loading playlist');
      callback(error);
      return;
    }
...
Player.prototype.replaceQueueWithPlaylist = function (playlistURI, callback) {
  ...
  _this.removeAllTracksFromQueue(function (error) {
    ...
    _this.addPlaylistToQueue(playlistURI, callback);
  });
};
...
Player.prototype.getLED = function (callback) {
  var soapCallback = function (error, res) {
    var buffer = [];
    res.setEncoding('utf-8');
